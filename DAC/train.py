...
# train과 test 폴더의 이미지를 데이터와 라벨로 분류합니다.
...
# 이미지를 모두 300x300x3으로 리사이징 후 numpy array를 구합니다.
...
image_pool_input = tf.placeholder(shape=[None, 300, 300, 3], dtype=tf.float32, name='image_pool_input')
...
# Deep Adaptive Image Clustering 비용함수 최적화
...
if mode == 'Training':
    ...
    with tf.Session() as sess:
        ...
        for i in range(65):
            data_samples = get_batch_test(batch_size, image_data, i)
            feed_dict = {image_pool_input: data_samples}
            pred_cluster = sess.run(pred_label, feed_dict=feed_dict)
